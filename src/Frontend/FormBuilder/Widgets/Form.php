...
class Form extends FrontendBaseWidget
{
    ...
    public function execute(): void
    {
        ...
        $this->loadData();
        $templateForm = $this->assignTemplateForm();
        $this->loadTemplate($templateForm);
    ...
    private function assignTemplateForm(): string
    {
        ...
        if (!empty($this->item)) {
            ...
            try {
                ...
                if ($theme !== 'core') {
                    $template = FRONTEND_PATH . '/Themes/' . $theme . '/Modules/FormBuilder/Layout/Widgets/'.$this->item['form_template'];  

                } else {
                    $template = FrontendTheme::getPath(FRONTEND_MODULES_PATH . '/FormBuilder/Layout/Widgets/'.$this->item['form_template']);            
                }
            ...
            $this->item['widgetForm'] = array();
        }

        return $template;
    }    
    
    public static function getTemplatesForm(): array
    {
        $templates = [];
        $finder = new Finder();
        $finder->name('*.html.twig');
        $finder->in(FRONTEND_MODULES_PATH . '/FormBuilder/Layout/Widgets');

        // if there is a custom theme we should include the templates there also
        $theme = BackendModel::get('fork.settings')->get('Core', 'theme', 'Fork');
        if ($theme !== 'core') {
            $path = FRONTEND_PATH . '/Themes/' . $theme . '/Modules/FormBuilder/Layout/Widgets';
            if (is_dir($path)) {
                $finder->in($path);
            }
        }
        ...
        if (!empty($this->item) && !empty($this->item['fields'])) {
            ...
            foreach ($this->item['fields'] as $field) {
                ...
                if ($field['type'] === 'dropdown') {
                    ...
                    // create element
                    $ddm = $this->form->addDropdown($item['name'], $values, $defaultIndex, false);
                    ...
                    // create element
                    $rbt = $this->form->addRadiobutton($item['name'], $values, $defaultValues);
                    ...
                    // create element
                    $chk = $this->form->addMultiCheckbox($item['name'], $newValues, $defaultValues);

                    // get content
                    $item['html'] = $chk->parse();
                } elseif ($field['type'] === 'textbox') {
                    // create element
                    $txt = $this->form->addText($item['name'], $defaultValues, 255);
                    ...
                    if ($field['settings']['input_type'] === 'date') {
                        ...
                        $datetime = $this->form->addText($item['name'], $defaultValues, 255, 'inputDatefield form-control fork-form-text ')->setAttributes(
                            [
                        ...
                        $datetime = $this->form->addText($item['name'], $defaultValues, 255)->setAttributes(['type' => 'time']);
                    }
                    ...
                    // create element
                    $txt = $this->form->addTextarea($item['name'], $defaultValues);
